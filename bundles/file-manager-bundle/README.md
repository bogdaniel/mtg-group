# FileBundle (Media File Manager for Symfony)

[![CI](https://github.com/zenchron/dashboard-bundle/actions/workflows/ci.yaml/badge.svg)](https://github.com/zenchron/dashboard-bundle/actions/workflows/ci.yaml)
...
return [
    // ...
    Ranky\FileBundle\ZenchronFileBundle::class => ['all' => true],
];
```
...
```yaml 
# config/routes/zenchron_file.yaml
zenchron_file:
  resource: "@ZenchronFileBundle/config/routes.php"
...
```php
# config/routes/zenchron_file.php
use Symfony\Component\Routing\Loader\Configurator\RoutingConfigurator;

return static function (RoutingConfigurator $routes) {
    $routes
        ->import('@ZenchronFileBundle/config/routes.php', 'annotation')
        ->prefix('/admin')
        ;
};
```
...
YAML
```yaml 
# config/packages/zenchron_file.yaml
zenchron_file:
...
This is the default configuration for FlysystemBundle. The storage name `zenchron_file.storage` is **mandatory** in case you want to change the configuration.
...
return static function (FlysystemConfig $flysystemConfig, ContainerConfigurator $container): void {
    $flysystemConfig
        ->storage('zenchron_file.storage') // Mandatory
        ->adapter('aws')
        ->options([
...
This configuration will also create a global twig variable (`zenchron_file_api_prefix`) that you can use later in your templates.
...
### Security
All routes in this bundle start with `/zenchron/dashboard` without any kind of security.
...
# config/packages/security.yaml
  ...
  access_control:
    ...
    - { path: ^/zenchron/dashboard, roles: ROLE_USER }
...
# Note: Only the *first* access control that matches will be used
  access_control:
    - { path: ^/admin/zenchron/dashboard, roles: ROLE_USER }
    - { path: ^/admin,     roles: ROLE_USER }
    - { path: ^/admin/users, roles: ROLE_SUPER_ADMIN }
```
...
Once installed, you can access a route that I have created for quick access to the Media File Manager. The path is `/zenchron/dashboard/embed`.
...
{% block stylesheets %}
    {{- parent() -}}
    {{ encore_entry_link_tags('zenchron_file', null, 'zenchron_file') }}
{% endblock %}

{% block javascripts %}
    {{- parent() -}}
    {{ encore_entry_script_tags('zenchron_file', null, 'zenchron_file', attributes={
        defer: true
    }) }}
{% endblock %}

{% block body %}
    <div class="ranky-media" data-api-prefix="{{ zenchron_file_api_prefix }}"></div>
{% endblock %}
```
...
```twig
{# @var media \Ranky\FileBundle\Application\DataTransformer\Response\MediaResponse #}
{% set media = zenchron_file.findById(page.mediaId) %}
{{ dump(media) }}
{% if media %}
  <p>
...
```twig
{% set medias = zenchron_file.findByIds(page.gallery) %}
{% for media in medias %}
  {# @var media \Ranky\FileBundle\Application\DataTransformer\Response\MediaResponse #}
  {# ... #}
{% endfor %}
...
```html
<button class="ranky-media-open-modal" data-api-prefix="{{zenchron_file_api_prefix}}" data-modal-title="Media File Manager" data-multiple-selection="true">
  Media File Manager
</button>
```
```js
...
<div class="form-group">
    <button type="button"
            ...
            data-api-prefix="{{ zenchron_file_api_prefix }}"
...
```php
use Zenchron\FileBundle\Presentation\Form\EasyAdmin\EARankyMediaFileManagerField;
...
FileBundle is licensed under the MIT License – see the [LICENSE](LICENSE) file for details
